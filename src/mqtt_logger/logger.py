...
class MqttLogger:
        ...
        try:
            ...
            while not self._shutdown_in_progress:
                try:
                    ...
                    try:
                        await asyncio.wait_for(self._flush_event.wait(), self.batch_interval)
                    except asyncio.TimeoutError:
    ...
    async def shutdown(self, timeout: float = 10.0) -> None:
        ...
        try:
            ...
            self._log_info(f"Logger shutdown complete. Published: {self._total_logs_published}, "
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "buffer_utilization": len(self._log_buffer) / self.max_buffer_size if self.max_buffer_size > 0 else 0.0,
    ...
    def from_env(cls, prefix: str = "MQTT_LOGGER_", **kwargs):
        ...
        config = {
            ...
            "log_level": level_map.get(os.environ.get(f"{prefix}LOG_LEVEL", "INFO"), logging.INFO),
            ...
            "reconnect_interval": int(os.environ.get(f"{prefix}RECONNECT_INTERVAL", "5")),
            "max_reconnect_attempts": int(os.environ.get(f"{prefix}MAX_RECONNECT_ATTEMPTS", "-1")),
            "throttle_interval": float(os.environ.get(f"{prefix}THROTTLE_INTERVAL", "0.1")),
        }
    ...
    def _log(self, level: int, message: str, data: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Log to file if enabled
        if self.log_file:
            logging.log(level, message, exc_info=data.get("exc_info", None) if data else None)
        ...
        if len(self._log_buffer) >= self.batch_size:
            ...
            if not self._shutdown_in_progress:
                asyncio.create_task(self._ensure_flush_task_running())
                ...
                if should_start_task and not self._shutdown_in_progress:
                    ...
                    try:
                        self._flush_task = asyncio.create_task(
                            self._delayed_flush(),
                            name=f"mqtt_logger_flush_{id(self)}"
                        )
    ...
    def _log_to_systemd_journal(self, level: int, message: str, data: Optional[Dict[str, Any]] = None) -> None:
    ...
    async def get_task_health(self) -> Dict[str, Any]:
        ...
        task_info = {
            ...
            "flush_task_cancelled": self._flush_task.cancelled() if self._flush_task else None,
            "task_restart_count": self._task_restart_count,
            "shutdown_in_progress": self._shutdown_in_progress,
            "buffer_size": len(self._log_buffer),
